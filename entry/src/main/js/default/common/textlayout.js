export default class textlayout {
     ...
     */
    static getTextHeight(str, widthPx=322) {
        return 40 * this.getTextLayout(str, widthPx) + 50;
    }
    ...
    static getTextLayout(str, widthPx=322) {
        ...
        while (i < str.length) {
            ...
            charPx = this.getCharPx(charCode);
            ...
            if (charCode >= 0xD800 && charCode <= 0xDBFF) { //对于双码点字符（如生僻字，emoji等的）特殊判断，因为jerryscript未更新到es6
                i+=2; //跳过下一个字节，即实际上跳过整个字符。
            } else {
                i++; //单码点字符正常处理
            }
        }
